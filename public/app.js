...
async function renderStudyRoom(){
  ...
  const mine=data.reservations.filter(r=>r.mine);
   const selectedDate=today(),selectedHours=studyHoursForDate(data.hours,selectedDate);
   $('#view').innerHTML=`<div class="card study-hero"><span class="eyebrow">📖 SALA DE ESTUDIOS</span><h2>Reserva tu bloque</h2><p>Una sala · bloques de hasta 4 horas · cancelación disponible siempre.</p>
    <form id="study-form" class="study-form"><label>Fecha<input id="study-date" type="date" min="${today()}" value="${selectedDate}" required></label><div id="study-hours-fields" class="study-hours-fields">${selectedHours?`<label>Desde<input id="study-start" type="time" min="${selectedHours.start}" max="${selectedHours.end}" value="${selectedHours.start}" required></label><label>Hasta<input id="study-end" type="time" min="${selectedHours.start}" max="${selectedHours.end}" value="${selectedHours.start==='09:00'?'10:00':selectedHours.end}" required></label>`:'<p class="study-closed" role="status">Sala de estudio cerrada hoy. No se puede reservar sábado ni domingo.</p>'}</div><div class="study-actions"><button id="study-submit" class="button primary" ${selectedHours?'':'disabled'}>Reservar</button></div></form></div>
   ...
   $('#study-form').onsubmit=reserveStudy;$('#study-date').onchange=()=>updateStudyDateControls(data.hours);$$('.study-cancel').forEach(b=>b.onclick=()=>cancelStudy(Number(b.dataset.id)));
}
function studyHoursForDate(hours,date){const day=new Date(`${date}T12:00:00Z`).getUTCDay();return hours?.[day]||null}
function updateStudyDateControls(hours){const d=$('#study-date')?.value,h=studyHoursForDate(hours,d),fields=$('#study-hours-fields'),submit=$('#study-submit');if(!fields||!submit)return;if(!h){fields.innerHTML='<p class="study-closed" role="status">Sala de estudio cerrada ese día. No se puede reservar sábado ni domingo.</p>';submit.disabled=true;return}fields.innerHTML=`<label>Desde<input id="study-start" type="time" min="${h.start}" max="${h.end}" value="${h.start}" required></label><label>Hasta<input id="study-end" type="time" min="${h.start}" max="${h.end}" value="${h.start==='09:00'?'10:00':h.end}" required></label>`;submit.disabled=false}
async function reserveStudy(e){e.preventDefault();const d=$('#study-date').value,a=$('#study-start')?.value,b=$('#study-end')?.value;if(!d||!a||!b)return;try{await api('/api/study-room/reserve',{method:'POST',body:{date:d,startTime:a,endTime:b}});toast('Sala de estudios reservada.');await renderStudyRoom()}catch(err){toast(err.message,true)}}
async function cancelStudy(id){if(!confirm('¿Cancelar esta reserva de Sala de estudios?'))return;try{await api('/api/study-room/reserve',{method:'DELETE',body:{id}});toast('Reserva cancelada.');await renderStudyRoom()}catch(err){toast(err.message,true)}}
...
function adminMemberDetailHtml(d){
  ...
  <div class="admin-member-detail-section source-applied"><div class="admin-section-title"><div><span class="eyebrow">SQLITE MI ASPCH</span><h4>Estado aplicado por Mi ASPCH</h4></div>${adminMemberStateBadge(applied.state)}</div><div class="admin-member-detail-fields">${field('Estado usado',availableAdminValue(applied.state))}${field('Estado financiero local',availableAdminValue(applied.financialState))}${field('Indicador active',applied.activeFlag?'Sí':'No')}${field('Restricciones activas',(applied.blockedModules||[]).join(', ')||'Ninguna')}</div><p class="hint">Causa: ${escapeHtml(applied.cause||'No disponible')}</p><div class="admin-access-columns"><div><strong>Módulos permitidos</strong><p>${escapeHtml((applied.allowedModules||[]).join(', ')||'Ninguno')}</p></div><div><strong>Módulos bloqueados</strong><p>${escapeHtml((applied.blockedModules||[]).join(', ')||'Ninguno')}</p></div></div></div>
   <div class="admin-member-detail-section"><span class="eyebrow">IDENTIDAD</span><h4>Nombre oficial y nombre de uso</h4><div class="admin-member-detail-fields">${field('Nombre oficial · BD SOCIOS',availableAdminValue(personal.officialName||m.officialName))}${field('Nombre de uso editable',availableAdminValue(personal.preferredName||m.preferredName))}</div><p class="hint">El nombre oficial proviene de BD SOCIOS y no se edita desde Mi ASPCH. El nombre de uso solo sirve para saludos; si falta una corrección oficial, debe hacerse en la fuente y sincronizarse.</p></div>
   <div class="admin-member-detail-section"><span class="eyebrow">BD SOCIOS</span><h4>Datos personales</h4><div class="admin-member-detail-fields">${field('RUT',availableAdminValue(personal.rut))}${field('Email',availableAdminValue(personal.email))}${field('Teléfono',availableAdminValue(personal.phone))}${field('Empresa / institución',availableAdminValue(personal.employer))}${field('Categoría',availableAdminValue(personal.category))}${field('Cargo',availableAdminValue(personal.position))}</div><p class="hint">Última actualización del snapshot: ${personal.lastSyncAt?formatLocalDateTime(personal.lastSyncAt):'No disponible'}. BD SOCIOS aporta identidad y datos personales.</p></div>
