    ...
    if(expected.kind==='inactive'){
      ...
      if(expected.kind==='admin'){
        ...
        if(expected.label==='ACTIVO'){
          ...
          await frame.evaluate(()=>go('simulators'));await frame.locator('.sim-tab[data-sim="a320pro"]').click();await frame.locator('[data-testid="a320pro-rate-2"]').waitFor();assert.match(await frame.locator('[data-testid="a320pro-rate-2"]').innerText(),/2 horas[\s\S]*\$75\.000/);assert.match(await frame.locator('[data-testid="a320pro-rate-4"]').innerText(),/4 horas[\s\S]*\$100\.000/);assert.equal(await frame.locator('#a320pro-request-form').count(),1);assert.equal(await frame.evaluate(()=>state.simulators.a320ProRequestUrl),'https://forms.gle/qzXaCUJgmTyufdKQA');await frame.locator('#a320pro-request-form [name="date"]').fill(await frame.evaluate(()=>new Date(Date.now()+86400000).toISOString().slice(0,10)));const officialFormPage=context.waitForEvent('page');await frame.locator('#a320pro-request-form button[type="submit"]').click();const opened=await officialFormPage;await opened.waitForLoadState('domcontentloaded').catch(()=>{});assert.match(opened.url(),/^https:\/\/(?:forms\.gle\/qzXaCUJgmTyufdKQA|docs\.google\.com\/forms\/)/);await opened.close();
