  ...
  for(const expected of profiles){
    ...
    if(expected.kind==='inactive'){
      ...
      if(expected.kind==='admin'){
        ...
        await frame.evaluate(()=>go('home'));await frame.locator('#view .hero h1').waitFor({timeout:10000});assert.match(await frame.locator('#view').innerText(),/Credencial vigente/);assert.equal(await frame.locator('#exit-simple-mode').count(),1,'Modo simple debe ofrecer salida discreta en Inicio');const navText=(await frame.locator('#desktop-nav,#mobile-nav').allInnerTexts()).join(' ');for(const allowed of ['Inicio','Estac.','Contacto'])assert.match(navText,new RegExp(allowed));for(const hidden of ['Reservas','Simuladores','Perfil','Biblioteca','Noticias','Convenios','Votaciones','Cursos','Credencial'])assert.doesNotMatch(navText,new RegExp(hidden));
      }else{
