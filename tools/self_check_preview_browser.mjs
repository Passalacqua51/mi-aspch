    ...
    if(expected.kind==='inactive'){
      ...
      if(expected.kind==='admin'){
        ...
        if(expected.label==='ACTIVO'){
          const homeText=await frame.locator('#view').innerText();assert.match(homeText,/EMERGENCIA \/ IFALPA/);assert.ok(await frame.locator('[data-testid="home-emergency"]').count());assert.equal(await frame.locator('[data-testid="home-emergency"] a[href*="ASPCH-EN-CASO-DE-ACCIDENTE-O-INCIDENTE.pdf"]').count(),1);assert.equal(await frame.evaluate(()=>state.config.push.enabled),false,'Preview no debe usar Push real');
          await frame.evaluate(()=>go('simulators'));await frame.locator('.sim-tab[data-sim="a320pro"]').click();await frame.locator('[data-testid="a320pro-rate-2"]').waitFor();assert.match(await frame.locator('[data-testid="a320pro-rate-2"]').innerText(),/2 horas.*\$75\.000/);assert.match(await frame.locator('[data-testid="a320pro-rate-4"]').innerText(),/4 horas.*\$100\.000/);assert.equal(await frame.locator('#a320pro-request-form').count(),1);assert.equal(await frame.evaluate(()=>state.simulators.a320ProRequestUrl),'https://forms.gle/qzXaCUJgmTyufdKQA');
          await frame.evaluate(()=>go('contact'));await frame.locator('.contact-channels-grid').waitFor();assert.equal(await frame.locator('.contact-hero h2').count(),0);assert.equal(await frame.locator('.official-resources a[href*="ASPCH-EN-CASO-DE-ACCIDENTE-O-INCIDENTE.pdf"]').count(),1);
        }
        if(expected.label==='ACTIVO')await frame.locator('#mobile-nav [data-view="profile"]').click();else await frame.evaluate(()=>go('profile'));await frame.locator('.profile-page').waitFor();assert.match(await frame.locator('.profile-page').innerText(),/Personalizar servicios/);if(expected.label==='ACTIVO'){await frame.locator('#top-avatar').click();await frame.locator('[data-account-view="membership"]').click()}else await frame.evaluate(()=>go('membership'));await frame.locator('#membership-view').waitFor();assert.doesNotMatch(await frame.locator('#view').innerText(),/Cargando…/);
