...
db.exec(`
CREATE TABLE IF NOT EXISTS member_financial_status(member_id INTEGER PRIMARY KEY REFERENCES members(id) ON DELETE CASCADE,financial_status TEXT,months_due INTEGER DEFAULT 0,amount_due INTEGER DEFAULT 0);
...
db.prepare(`INSERT INTO member_financial_status(member_id,financial_status,months_due,amount_due) VALUES (?,'MOROSO',2,30000)`).run(m1.id);
initV060(db);
