...
for(const source of [index,app,css,sw,manifest])assert.ok(!/logo-aspch-(?:full|mark|dark|mark-dark|white)\.png/.test(source),'No deben sobrevivir referencias visuales a logos anteriores');
assert.ok(index.match(/\/logo-aspch-original\.png/g)?.length>=3,'Login y desbloqueo deben usar el logo oficial');
assert.deepEqual(pngSize('apple-touch-icon.png'),[180,180]);assert.deepEqual(pngSize('icon-192.png'),[192,192]);assert.deepEqual(pngSize('icon-512.png'),[512,512]);
assert.ok(index.includes('rel="apple-touch-icon" sizes="180x180" href="/apple-touch-icon.png"')&&manifest.includes('"/icon-192.png"')&&manifest.includes('"/icon-512.png"'),'La PWA debe declarar iconos cuadrados válidos para iPhone y demás plataformas');
...
assert.match(sw,/keep-parking/);assert.match(sw,/vacate-parking/);assert.match(server,/actions:\[\{action:'keep-parking',title:'Sí, sigo aquí'\},\{action:'vacate-parking',title:'No, ya desocupé'\}\]/);
assert.ok(server.includes("p === '/api/parking/still-active'")&&app.includes('showParkingConfirmationPrompt'),'La confirmación Push debe registrar “sigue aquí” y ofrecer Sí/No al abrirla en iPhone');
...
assert.ok(app.includes('notificationConsentKey()')&&app.includes('syncPushSubscription()'),'El consentimiento Push debe quedar asociado al dispositivo y preceder la suscripción');
...
assert.ok(index.includes('href="/informatica">Acceso Informática</a>')&&index.includes('id="admin-mode-switch"')&&read('public/admin.html').includes('id="admin-mode-switch"'),'El enlace público y ambas vistas deben exponer el cambio seguro de modo ADMIN');
...
assert.ok(app.includes('home-emergency')&&app.includes('Abrir protocolo PDF')&&server.includes('ASPCH-EN-CASO-DE-ACCIDENTE-O-INCIDENTE.pdf'),'Inicio debe exponer acciones de emergencia/IFALPA con fuente oficial');
assert.ok(server.includes('ADMIN_PUSH_TEMPLATES')&&server.includes('ADMIN_PUSH_AUDIENCES')&&server.includes("audience==='SELF'")&&server.includes("is_board=1"),'Push ADMIN debe usar plantillas allowlist y limitarse a Directorio o Informática');
assert.ok(!server.includes("p === '/api/auth/request-code'")&&!server.includes("p === '/api/auth/verify-code'"),'No deben sobrevivir endpoints de autenticación por correo legado');
