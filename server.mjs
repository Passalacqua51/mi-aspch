...
async function routeApi(req, res, url) {
  ...
  if (p.startsWith('/api/admin/')) {
    ...
    if(req.method==='GET'&&p==='/api/admin/finance'){
      const financial=await readFinancialSafe();
      const debtors=db.prepare(`SELECT m.id,m.name,f.financial_status,f.months_due,f.amount_due,f.source_year,f.synced_at FROM member_financial_status f JOIN members m ON m.id=f.member_id WHERE (COALESCE(f.months_due,0)>0 OR COALESCE(f.amount_due,0)>0) ORDER BY COALESCE(f.amount_due,0) DESC,COALESCE(f.months_due,0) DESC LIMIT 200`).all()
        .map(r=>({id:Number(r.id),name:r.name,financialStatus:r.financial_status||'No disponible',monthsDue:Number(r.months_due||0),amountDue:Number(r.amount_due||0),sourceYear:r.source_year||null,syncedAt:r.synced_at||null}));
      return json(res,200,{generatedAt:new Date().toISOString(),mode:'READ_ONLY',source:{ready:!!financial.sourceReady,status:financial.status||'UNKNOWN',fileName:financial.fileName||null,sheet:financial.sheet||null,lastReadAt:financial.lastReadAt||null,sourceModifiedAt:financial.sourceModifiedAt||null,rows:Number(financial.rows||0),validRows:Number(financial.validRows||0),formulaErrors:Number(financial.formulaErrors||0),counts:financial.counts||{},error:financial.error||null},lastSync:latestFinancialSync(db),debtors,debtorsTotal:db.prepare(`SELECT COUNT(*) n FROM member_financial_status WHERE (COALESCE(months_due,0)>0 OR COALESCE(amount_due,0)>0)`).get().n||0});
    }
