...
const STUDY_ROOM_HOURS=Object.freeze({
  1:Object.freeze({start:'09:00',end:'17:00'}),
  2:Object.freeze({start:'09:00',end:'17:00'}),
  3:Object.freeze({start:'09:00',end:'17:00'}),
  4:Object.freeze({start:'09:00',end:'17:00'}),
  5:Object.freeze({start:'09:00',end:'16:00'})
});
...
export function studyRoomHours(){return STUDY_ROOM_HOURS}

function chileDateParts(value){
  const date=value instanceof Date?value:new Date(value);
  if(!Number.isFinite(date.getTime()))return null;
  const parts=Object.fromEntries(new Intl.DateTimeFormat('en-CA',{timeZone:TZ,weekday:'short',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(date).filter(x=>x.type!=='literal').map(x=>[x.type,x.value]));
  const weekdays={Sun:0,Mon:1,Tue:2,Wed:3,Thu:4,Fri:5,Sat:6};
  return{date:`${parts.year}-${parts.month}-${parts.day}`,weekday:weekdays[parts.weekday]};
}

export function studyRoomScheduleFor(start){
  const parts=chileDateParts(start);return parts?STUDY_ROOM_HOURS[parts.weekday]||null:null;
}

export function validateStudyRoomSchedule(start,end){
  const a=new Date(start),b=new Date(end),hours=studyRoomScheduleFor(a);
  if(!hours)throw Object.assign(new Error('La Sala de estudios está cerrada los sábados y domingos.'),{statusCode:400,expose:true});
  const localStart=chileDateParts(a),localEnd=chileDateParts(b);
  if(!localEnd||localStart.date!==localEnd.date)throw Object.assign(new Error('La reserva debe comenzar y terminar el mismo día.'),{statusCode:400,expose:true});
  const startMinutes=Number(new Intl.DateTimeFormat('en-GB',{timeZone:TZ,hour:'2-digit',minute:'2-digit',hour12:false}).format(a).split(':')[0])*60+Number(new Intl.DateTimeFormat('en-GB',{timeZone:TZ,hour:'2-digit',minute:'2-digit',hour12:false}).format(a).split(':')[1]);
  const endText=new Intl.DateTimeFormat('en-GB',{timeZone:TZ,hour:'2-digit',minute:'2-digit',hour12:false}).format(b),endParts=endText.split(':'),endMinutes=Number(endParts[0])*60+Number(endParts[1]);
  const [openHour,openMinute]=hours.start.split(':').map(Number),[closeHour,closeMinute]=hours.end.split(':').map(Number);
  if(startMinutes<openHour*60+openMinute||endMinutes>closeHour*60+closeMinute)throw Object.assign(new Error(`La Sala de estudios funciona de ${hours.start} a ${hours.end} ese día.`),{statusCode:400,expose:true});
  return hours;
}
...
export function studyRoomAvailability(db,{from,to,memberId}){
  ...
  return {rooms,hours:STUDY_ROOM_HOURS,reservations:rows.map(r=>({id:r.id,roomId:r.room_id,room:r.name,start:r.start_at,end:r.end_at,mine:r.member_id===memberId,status:r.member_id===memberId?'Tu reserva':'Ocupada'}))};
}
...
export function reserveStudyRoom(db,{memberId,start,end}){
  ...
  validateStudyRoomSchedule(start,end);
