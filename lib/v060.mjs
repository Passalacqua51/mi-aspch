...
const ADMIN_MEMBER_STATES=new Set(['MOROSO','CONGELADO','DESAFILIADO']);
function adminMemberState(row){
  const financial=String(row?.financial_status||'').trim().toUpperCase();
  if(ADMIN_MEMBER_STATES.has(financial))return financial;
...
export function adminMembersList(db,{query='',page=1,limit=20}={}){
  ...
  const rows=db.prepare(`SELECT m.id,m.name,m.email,m.rut,m.active,f.financial_status FROM members m LEFT JOIN member_financial_status f ON f.member_id=m.id WHERE ${clause} ORDER BY m.name COLLATE NOCASE,m.id LIMIT ? OFFSET ?`).all(...args,safeLimit,offset);
  return{query:q,page:current,limit:safeLimit,total,pages,members:rows.map(row=>({id:Number(row.id),name:row.name,rutMasked:maskedRut(row.rut),emailMasked:maskedEmail(row.email),membershipState:adminMemberState(row)}))};
}

export function adminMemberDetail(db,id){
  id=Number(id);if(!Number.isSafeInteger(id)||id<=0)return null;
  const row=db.prepare(`SELECT m.id,m.name,m.rut,m.email,m.phone,m.employer,m.active,m.updated_at,f.source_status,f.financial_status,f.months_due,f.amount_due,f.source_year,f.source_updated_at,f.synced_at FROM members m LEFT JOIN member_financial_status f ON f.member_id=m.id WHERE m.id=? AND m.role!='ADMIN'`).get(id);
  if(!row)return null;
  const now=nowIso(),today=new Intl.DateTimeFormat('en-CA',{timeZone:'America/Santiago',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date());
  ...
  const hasFinancial=!!row.financial_status;
  return{
    member:{id:Number(row.id),name:row.name,rut:row.rut||null,email:row.email||null,phone:row.phone||null,employer:row.employer||null,membershipState:adminMemberState(row)},
    financial:{status:hasFinancial?row.financial_status:null,sourceStatus:hasFinancial?(row.source_status||null):null,monthsDue:hasFinancial?Number(row.months_due||0):null,amountDue:hasFinancial?Number(row.amount_due||0):null,sourceYear:hasFinancial?(row.source_year||null):null},
    security:{activeSessions:Number(db.prepare('SELECT COUNT(*) n FROM sessions WHERE member_id=? AND expires_at>?').get(id,now).n||0),passkeys:Number(db.prepare('SELECT COUNT(*) n FROM passkeys WHERE member_id=?').get(id).n||0)},
    reservations:[...parking,...study].sort((a,b)=>String(a.date||a.start).localeCompare(String(b.date||b.start))),
    lastUpdate:{memberUpdatedAt:row.updated_at||null,financialSyncedAt:hasFinancial?(row.synced_at||null):null,financialSourceUpdatedAt:hasFinancial?(row.source_updated_at||null):null,latestAvailable:latestIso(row.updated_at,hasFinancial?row.synced_at:null,hasFinancial?row.source_updated_at:null)}
  };
}
