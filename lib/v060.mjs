...
export function adminReservationsSnapshot(db,{today}={}){
  ...
  return{
    ...
    summary:{
      parkingSpaces:safeDbCount(db,"SELECT COUNT(*) n FROM parking_spaces WHERE active=1 AND board_only=0"),
      parkingToday:safeDbCount(db,"SELECT COUNT(*) n FROM parking_reservations WHERE status='ACTIVE' AND reservation_date=?",date),
      parkingActive:safeDbCount(db,"SELECT COUNT(*) n FROM parking_reservations WHERE status='ACTIVE' AND reservation_date>=?",date),
    ...
    parking:{today:parkingToday,active:parkingActive},study:{active:study,waitlist},
...
export function adminMemberDetail(db,id,{authority=null,modules={},otpDeliveryEnabled=false}={}){
  ...
  return{
    member:{id:Number(row.id),name:row.name,rut:row.rut||null,email:row.email||null,phone:row.phone||null,employer:row.employer||null,membershipState:appliedState},
    ...
    personal:{source:'BD SOCIOS',rut:row.rut||null,email:row.email||null,phone:row.phone||null,employer:row.employer||null,category:row.category||null,position:row.position||null,birthDate:row.birth_date||null,joinedAt:row.joined_at||null,lastSyncAt:row.updated_at||null},
    ...
    actions:{closeSessions:sessions.length>0,revokePasskeys:Number(passkeySummary.n||0)>0,resetPin:!!row.pin_hash,issueOtp:!!otpDeliveryEnabled,releaseParking:parking.length>0,cancelStudy:study.length>0,refresh:true},
