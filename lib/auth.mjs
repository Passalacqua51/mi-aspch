...
export async function issueOtp(db, { memberId=null, email, purpose='login', mailKind='login' }) {
  ...
  return { ok:true, delivered, expiresAt:expires.toISOString() };
}
...
export function sessionCookieName() { return process.env.SESSION_COOKIE_NAME || 'mi_aspch_session'; }

function sessionToken(req) { return parseCookies(req.headers.cookie || '')[sessionCookieName()] || ''; }

export function memberFromRequest(db, req, { allowInactive = false } = {}) {
  ...
  const inactiveAllowed = typeof allowInactive === 'function' ? !!allowInactive(row) : !!allowInactive;
  if (!row || (!row.active && !inactiveAllowed) || new Date(row.session_expires).getTime() < Date.now()) return null;
  return row;
}
