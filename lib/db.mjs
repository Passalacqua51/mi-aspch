...
const PREFERRED_NAME_OFFENSIVE_TOKENS = new Set(['baboso','caca','concha','chucha','culiao','culo','mierda','paja','pene','puta','puto','tula','verga','weon','weona']);

function normalizedNameToken(value){
  return String(value||'').normalize('NFKD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
}

export function validatePreferredName(preferredName){
  const value = String(preferredName || '').normalize('NFKC').trim().replace(/\s+/g, ' ');
  if (!value) return { ok:false, reason:'empty' };
  if (value.length > 40) return { ok:false, reason:'too_long' };
  if (/https?:\/\/|www\.|[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}|(?:\+?\d[\s().-]*){7,}/i.test(value)) return { ok:false, reason:'contact_or_url' };
  if (!/^[\p{L}\p{M} .'-]+$/u.test(value)) return { ok:false, reason:'invalid_chars' };
  const tokens=value.split(' ').filter(Boolean).map(normalizedNameToken);
  if (tokens.some(token=>!token||/^([a-z])\1{3,}$/.test(token))) return { ok:false, reason:'nonsense' };
  if (tokens.some((token,index)=>tokens.indexOf(token)!==index&&tokens.filter(item=>item===token).length>=3)) return { ok:false, reason:'nonsense' };
  if (tokens.some(token=>PREFERRED_NAME_OFFENSIVE_TOKENS.has(token))) return { ok:false, reason:'offensive' };
  if (tokens.length>=2&&tokens.every(token=>token.length===1)&&PREFERRED_NAME_OFFENSIVE_TOKENS.has(tokens.join(''))) return { ok:false, reason:'offensive' };
  return { ok:true, value };
}

export function updatePreferredName(db, memberId, preferredName) {
  const result=validatePreferredName(preferredName);
